...
void solve() {
	...
	vector<Point<long long>> majors;
    for (int i = 1; i <= n; i++) {
        long long x, y;
        cin >> x >> y;
        points[i] = Point(x, y);
        if (i <= k) {
            majors.emplace_back(points[i]);
        }
    }
	long long ans = 1LL * abs(points[a].x - points[b].x) + 1LL * abs(points[a].y - points[b].y);
    if (k == 0) {
        cout << ans << '\n';
        return;
    }
    KDTree tree(majors);
    ans = min(ans, 1LL * tree.nearest(points[a]).first + 1LL * tree.nearest(points[b]).first);
    cout << ans << '\n';
}
