...
struct Info {
    ...
    Info() : val({LLONG_MAX, -1}) {}
    Info(pair<long long, int> x) : val(x) {}
};
...
int main() {
    ...
    long long X = 0;
    vector<tuple<long long, int, int>> H;

    function<void(int, int, long long)> DnC = [&](int l, int r, long long minLast) {
        if (l > r) {
            return;
        }
        auto [mn, pos] = sgt.rangeQuery(l, r).val;
        if (mn > minLast) {
            H.emplace_back(X + 1, l, r);
            X += (mn - minLast) * (r - l + 1);
        }
        DnC(l, pos - 1, mn);
        DnC(pos + 1, r, mn);
    ...
    auto getBlock = [&](long long k) -> long long {
        int at = -1;
        {
