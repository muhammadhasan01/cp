...
int main() {
    ...
    vector<vector<bool>> m(k, vector<bool>(k));
    for (int i = 0; i < k; i++) {
        for (int j = 0; j < k; j++) {
            char cc;
            IO::read_char(cc);
    ...
    vector<vector<int>> adj(1 << k);
    vector<int> idx(1 << k, -1);
    vector<set<int>> bits;
    {
        vector<int> p(k, 1);
        for (int i = 0; i < X; i++) {
        ...
        for (auto& bit : bits) {
            ...
            adj[idx[id]].emplace_back(idx[id]);
            for (int i = 0; i < k; i++) {
                if (bit.count(i)) {
                    continue;
                }
                for (int j : bit) {
                    int cid = id + (1 << i) - (1 << j);
                    adj[idx[cid]].emplace_back(idx[id]);
                }
            } 
        }
    }
    ...
    IO::write_int(*max_element(dp.begin(), dp.end()), '\n');
