...
struct Pair {
    ...
    Pair() : val({-LLONG_MAX, 0}) {}
    Pair(pair<long long, int> v) : val(v) {}
};

Pair operator+(const Pair& lhs, const Pair& rhs) {
    return Pair(max(lhs.val, rhs.val));
}
...
int main() {
    ...
    SegmentTree<Pair> spA(A);
    SegmentTree<Info> spB(B);
    SegmentTree<Info> spC(C);
    ...
    auto solve = [&](int s, int t) -> long long {
        ...
        int m = spA.rangeQuery(s, t).val.second;
        long long up = a[m] - a[s];
        long long down = a[m] - a[t];
        long long maintain = 0;
        maintain += spB.rangeQuery(s, m).val - (a[m] - m);
        maintain += spC.rangeQuery(m, t).val - (a[m] - n + m - 1);
        if (inverted) {
            swap(up, down);
        }
        return 2LL * maintain + 4LL * up + 1LL * down;
    };
