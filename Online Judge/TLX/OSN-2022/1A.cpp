...
long long getLen(long long n, long long pos) {
    if (pos <= (n + 1) / 2) {
        return 2LL * pos - 1;
    }
    return getLen(n, n + 1 - pos) + 1;
}
...
long long solve(long long n, long long x) {
    ...
    if (x % p == 0) {
        long long val = x / p;
        long long len = val * 2;
        long long l = 1, r = (n + 1) / 2, at = -1;
        while (l <= r) {
            long long mid = (l + r) >> 1LL;
            long long cur = n - getLen(n, mid) + 1;
            if (cur >= len) {
                l = mid + 1;
                at = mid;
            } else {
        ...
        if (rem <= (n + 1) / 2) {
            long long val = x / p;
            long long len = val * 2 + 1;
            long long maxLen = getLen(n, rem);
            res += (len <= maxLen);
        }
    }
    long long q = n + 2;
