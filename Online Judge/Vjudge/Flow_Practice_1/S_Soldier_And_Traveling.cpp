...
int main() { 
    ...
    int flow = dinic.flow();
    if (soldier == total && flow == total) {
        cout << "YES" << '\n';
        vector<FlowEdge>& cur = dinic.edges;
        vector<vector<int>> answer(n, vector<int>(n));
        for (int i = 0; i < (int) cur.size(); i += 2) {
            if (cur[i].v == source || cur[i].u == sink) continue;
            answer[cur[i].v][cur[i].u - n] = cur[i].flow;
        }
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                cout << answer[i][j] << (j == n - 1 ? '\n' : ' ');
            }
        }
    } else {
        cout << "NO" << '\n';
